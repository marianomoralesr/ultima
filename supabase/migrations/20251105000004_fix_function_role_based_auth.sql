-- Fix get_secure_client_profile to use role-based authentication
-- This replaces hardcoded email list with dynamic role check from profiles table
-- Also removes reference to non-existent date_of_birth column
...
BEGIN
    ...
    -- Uses to_jsonb(p.*) to automatically include all profile columns
...
'Returns complete client profile with applications, tags, reminders, and documents. Includes advisor name when available. Only accessible to users with admin role (dynamically checked from profiles table). Uses SECURITY DEFINER to bypass RLS.';
