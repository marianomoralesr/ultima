...
-- This version removes the autorizar_asesor_acceso check since it may not exist
...
-- Admin and sales can view documents from leads they are assigned to
...
USING (
  ...
  -- Sales role can see documents from their assigned leads
  EXISTS (
...
USING (
  ...
  -- Sales role can update documents from their assigned leads
  EXISTS (
    SELECT 1 FROM public.profiles lead
    WHERE lead.id = uploaded_documents.user_id
      AND lead.asesor_asignado_id = auth.uid()
...
-- Sales can delete documents from their assigned leads
...
USING (
  ...
  -- Sales role can delete documents from their assigned leads
  EXISTS (
    SELECT 1 FROM public.profiles lead
    WHERE lead.id = uploaded_documents.user_id
      AND lead.asesor_asignado_id = auth.uid()
...
'Users can view their own documents. Admin role can view all. Sales role can view documents from leads they are assigned to.';

COMMENT ON POLICY "uploaded_documents_update" ON public.uploaded_documents IS
'Users can update their own documents. Admin role can update all. Sales role can update documents from leads they are assigned to.';

COMMENT ON POLICY "uploaded_documents_delete" ON public.uploaded_documents IS
'Users can delete their own documents. Admin role can delete all. Sales role can delete documents from leads they are assigned to.';
