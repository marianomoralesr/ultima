-- Recreate get_leads_for_dashboard function that was dropped by CASCADE
-- This function is used by SimpleCRMPage and AdminLeadsDashboardPage
...
'[PERMANENT] Returns leads for CRM dashboard. Admin sees all leads, Sales sees only assigned leads. Uses SECURITY DEFINER to bypass RLS.';

DO $$
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '=== get_leads_for_dashboard() FUNCTION RECREATED ===';
    RAISE NOTICE '';
    RAISE NOTICE 'Function signature: get_leads_for_dashboard() RETURNS TABLE(...)';
    RAISE NOTICE 'Security: SECURITY DEFINER (bypasses RLS)';
    RAISE NOTICE 'Access: Admin sees all leads, Sales sees only assigned leads';
    RAISE NOTICE '';
END $$;
