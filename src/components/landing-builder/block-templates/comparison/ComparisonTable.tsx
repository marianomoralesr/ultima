          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                {items.map(item => (
                  <th key={item.id} scope="col" className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                    {item.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {features.map(feature => (
                <tr key={feature.id}>
                  ...
                  {items.map(item => (
                    <td key={item.id} className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
