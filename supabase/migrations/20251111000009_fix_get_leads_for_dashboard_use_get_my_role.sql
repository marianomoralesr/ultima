-- Update get_leads_for_dashboard to use get_my_role() for consistency
...
COMMENT ON FUNCTION public.get_leads_for_dashboard() IS 'Returns leads for CRM dashboard with timestamps. Only returns profiles with role=user (excludes admin/sales users). Requires admin or sales role from profiles table. Uses get_my_role() to avoid recursion. Default sort is by last_sign_in_at DESC.';
